...
gcloud run deploy real-estate-portal --source . --region "$REGION" --allow-unauthenticated \
  --set-env-vars "SERVICE_API_KEY=$SERVICE_API_KEY,PORTAL_URL=$PORTAL_URL,SURVEY_URL=$SURVEY_URL,AD_URL=$AD_URL,GOOGLE_OAUTH_CLIENT_ID=$GOOGLE_OAUTH_CLIENT_ID,ADMIN_EMAILS=$ADMIN_EMAILS,FLASK_SECRET_KEY=$FLASK_SECRET_KEY,FREE_SEARCH_LIMIT=3,FREE_AD_LIMIT=3,GCS_BUCKET=$GCS_BUCKET" \
  --quiet
echo "Portal 部署完成"
echo ""
